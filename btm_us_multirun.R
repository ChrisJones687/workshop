...
setff("In", "C:/Users/cmjone25/Desktop/workshop_data/boxtree_moth/US_data/")

infected_file <- ffIn("CONUS_InfectionsTry2.tif")
host_file <- ffIn("buxus.tif")
...
parameter_means <- as.numeric(parameter_means)
parameter_cov_matrix <- read.csv(ffIn("posterior_cov_matrix.csv"))


btm_run <- pops_multirun(infected_file,
                         host_file,
...
plot(btm_run$probability[[3]])
plot(btm_run$simulation_mean[[10]])
plot(btm_run$probability[[3]])
